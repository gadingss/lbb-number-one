...
<head>
    ...
    <title>Sistem Bimbel Privat - Siswa</title>
...
<div class="flex min-h-screen">
    ...
    <aside class="w-64 bg-white shadow-xl">
        ...
        <div class="p-6 border-b">
            ...
            <p class="text-sm text-gray-500">PRIVAT - SISWA</p>
        </div>
        ...
        <nav class="mt-6 px-4 space-y-2">
            ...
            <a href="{{ route('siswa.dashboard') }}"
               ...
               {{ str_contains($route, 'siswa.dashboard') ? 'bg-blue-600 text-white' : 'hover:bg-gray-100 text-gray-700' }}">
                Dashboard
            </a>

            <a href="{{ route('siswa.paket.index') }}"
               class="block px-4 py-2 rounded-lg transition
               {{ str_contains($route, 'siswa.paket') ? 'bg-blue-600 text-white' : 'hover:bg-gray-100 text-gray-700' }}">
                Pilih Paket
            </a>

            <a href="{{ route('siswa.jadwal.index') }}"
               class="block px-4 py-2 rounded-lg transition
               {{ str_contains($route, 'siswa.jadwal') ? 'bg-blue-600 text-white' : 'hover:bg-gray-100 text-gray-700' }}">
                Jadwal Les
            </a>

            <a href="{{ route('siswa.riwayat.index') }}"
               class="block px-4 py-2 rounded-lg transition
               {{ str_contains($route, 'siswa.riwayat') ? 'bg-blue-600 text-white' : 'hover:bg-gray-100 text-gray-700' }}">
                Riwayat Les
            </a>
