...
@section('content')
<div class="mb-6">
    <h2 class="text-2xl font-bold text-gray-800">Jadwal Mengajar & Izin</h2>
    <p class="text-gray-500 text-sm mt-1">
        Kelola jadwal rutin mengajar dan persetujuan reschedule
    </p>
</div>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    ...
    <div class="lg:col-span-2">
        <div class="bg-white rounded-xl shadow-sm overflow-hidden mb-6">
            <div class="px-6 py-4 border-b bg-gray-50">
                <h3 class="text-lg font-bold text-gray-800">Jadwal Rutin</h3>
            </div>
            <div class="p-6">
                @php
                    ...
                    <div class="mb-6 last:mb-0">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div class="border rounded-lg p-4 relative border-l-4 border-l-blue-500">
                                ...
                                <p class="text-gray-500 text-sm mt-1">
                                    <i class="far fa-user mr-1"></i> Siswa: {{ $jadwal->siswa->user->name ?? 'Unknown' }}
                                </p>
                                @if($jadwal->siswa && $jadwal->siswa->latitude && $jadwal->siswa->longitude)
                                    <p class="text-blue-600 hover:text-blue-800 text-xs mt-1">
                                        <a href="https://www.google.com/maps/dir/?api=1&destination={{ $jadwal->siswa->latitude }},{{ $jadwal->siswa->longitude }}" target="_blank">
                                            <i class="fas fa-map-marker-alt mr-1"></i> Lihat Rute ke Lokasi Siswa
                                        </a>
                                    </p>
                                @else
                                    <p class="text-gray-400 text-xs mt-1 cursor-help" title="Siswa ini belum mengatur lokasi GPS di profilnya">
                                        <i class="fas fa-map-marker-alt mr-1"></i> Lokasi belum diatur siswa
                                    </p>
                                @endif
                ...
                @if($jadwals->isEmpty())
                    <p class="text-gray-500 text-center py-4">Belum ada jadwal mengajar tersedia.</p>
                @endif
            </div>
        </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                @forelse($jadwalPenggantis as $jp)
                    <div class="border rounded-lg p-4 bg-blue-50 border-blue-200">
                        ...
                        <p class="text-gray-500 text-sm mt-1">
                            <i class="far fa-user mr-1"></i> Siswa: {{ $jp->jadwal->siswa->user->name ?? 'Unknown' }}
                        </p>
                        @if($jp->jadwal->siswa && $jp->jadwal->siswa->latitude && $jp->jadwal->siswa->longitude)
                            <p class="text-blue-600 hover:text-blue-800 text-xs mt-1">
                                <a href="https://www.google.com/maps/dir/?api=1&destination={{ $jp->jadwal->siswa->latitude }},{{ $jp->jadwal->siswa->longitude }}" target="_blank">
                                    <i class="fas fa-map-marker-alt mr-1"></i> Lihat Rute ke Lokasi Siswa
                                </a>
                            </p>
                        @else
                        ...
                        <div class="mt-3 pt-3 border-t border-blue-200">
                            <a href="{{ route('tutor.absensi.create', ['jadwal' => 'pengganti_'.$jp->id]) }}" class="inline-block text-xs bg-blue-600 text-white px-3 py-1.5 rounded hover:bg-blue-700 font-medium">
                                Catat Absensi
                            </a>
                        </div>
                    </div>
    ...
    <div class="lg:col-span-1">
        <div class="bg-white rounded-xl shadow-sm p-6">
            <h3 class="text-lg font-bold text-gray-800 mb-4">Persetujuan & Status Izin</h3>
            
            <div class="space-y-4">
                @forelse($pengajuanIzins as $izin)
                    <div class="border rounded-lg p-4 {{ $izin->status == 'menunggu_lawan' && $izin->tipe_pengaju == 'siswa' ? 'border-orange-400 bg-orange-50' : 'bg-gray-50' }}">
                        ...
                        <p class="text-xs text-gray-600 mt-1">Siswa: <span class="font-semibold">{{ $izin->jadwal->siswa->user->name ?? 'Unknown' }}</span></p>
                        ...
                        <p class="text-xs text-gray-600 mt-1">Alasan: <i>"{{ $izin->alasan }}"</i></p>

                        {{-- JIKA TUTOR YG MENGAJUKAN, DAN MENUNGGU KONFIRMASI DARI TUTOR SENDIRI (SISWA SUDAH MENGUSULKAN) --}}
                        @if($izin->tipe_pengaju == 'tutor' && $izin->status == 'menunggu_konfirmasi_pengaju')
                            <div class="mt-3 pt-3 border-t border-gray-200">
                                <p class="text-xs font-bold text-gray-700 mb-2">Siswa mengusulkan jadwal pengganti:</p>
                                ...
                                <form action="{{ route('tutor.izin.accept', $izin->id) }}" method="POST" class="mt-2">
                                    @csrf
                        ...
                        {{-- JIKA SISWA YG MENGAJUKAN, DAN TUTOR YG HARUS APPROVE & PROPOSE --}}
                        @if($izin->tipe_pengaju == 'siswa' && $izin->status == 'menunggu_lawan')
                            <div class="mt-3 pt-3 border-t border-gray-200">
                                <p class="text-xs text-gray-600 mb-2">Siswa meminta izin. Anda perlu memberikan persetujuan dan jadwal pengganti.</p>
                                <div class="flex gap-2">
                                    ...
                                    <form action="{{ route('tutor.izin.reject', $izin->id) }}" method="POST" class="flex-1" onsubmit="return confirm('Tolak izin siswa?')">
                                        @csrf
...
<div id="izinModal" class="fixed inset-0 z-50 flex items-center justify-center hidden bg-black/50">
    <div class="bg-white p-6 rounded-xl shadow-xl w-full max-w-md">
        ...
        <form action="{{ route('tutor.izin.store') }}" method="POST">
            @csrf
...
{{-- MODAL PROPOSE RESCHEDULE (Approve izin Siswa) --}}
<div id="proposeModal" class="fixed inset-0 z-50 flex items-center justify-center hidden bg-black/50">
    <div class="bg-white p-6 rounded-xl shadow-xl w-full max-w-md">
        ...
        <p class="text-sm text-gray-600 mb-4">Pilih waktu yang cocok untuk mengganti kelas ini. Sistem otomatis memvalidasi bentrok dengan jadwal aktif.</p>
...
<script>
    ...
    function openProposeModal(izinId) {
        document.getElementById('proposeForm').action = '/tutor/izin/' + izinId + '/approve';
