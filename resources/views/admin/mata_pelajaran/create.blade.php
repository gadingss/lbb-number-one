...
<div class="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-8">
    <div>
        <h3 class="font-headline text-3xl font-extrabold text-on-surface tracking-tight">Tambah Mata Pelajaran</h3>
        <p class="text-on-surface-variant mt-1">Tambahkan mata pelajaran baru ke dalam sistem.</p>
    </div>
...
<div class="bg-surface-container-lowest rounded-[2rem] overflow-hidden max-w-3xl">
    <div class="p-8 bg-white/40 backdrop-blur-sm border-b border-surface-container-high">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h4 class="text-xl font-headline font-bold">Formulir Mata Pelajaran</h4>
                <p class="text-xs text-on-surface-variant">Detail kurikulum / pelajaran</p>
            </div>
        </div>
    </div>

    <form action="{{ route('admin.mata-pelajaran.store') }}" method="POST" class="p-8">
        @csrf
        ...
        <div class="grid grid-cols-1 gap-6 mb-8">
            ...
            <div>
                ...
                <textarea name="deskripsi" rows="4"
                    ...
                    placeholder="Penjelasan singkat mengenai mata pelajaran ini">{{ old('deskripsi') }}</textarea>
            </div>
        </div>
